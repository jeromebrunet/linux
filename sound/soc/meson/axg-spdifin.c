...
#define  SPDIFIN_CTRL0_STATUS_SEL_MASK	GENMASK(10, 8)
#define  SPDIFIN_CTRL0_STATUS_SEL(x)	((x) << 8)
#define SPDIFIN_CTRL1			0x04
#define  SPDIF_CTRL1_BASE_TIMER_MASK	GENMASK(19, 0)
#define  SPDIF_CTRL1_BASE_TIMER(x)	((x) << 0)
...
#define  SPDIFIN_STAT0_MODE(x)		(((x) >> 28) & GENMASK(2, 0))
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (ret) {
		dev_err(dai->dev, "failed to enable spdifin reference clock\n");
		return ret;
	}
					  ...
					  struct axg_spdifin *priv)
{
	...
	regmap_update_bits(priv->map, SPDIFIN_CTRL1,
			   SPDIF_CTRL1_BASE_TIMER_MASK,
			   SPDIF_CTRL1_BASE_TIMER(rate / 1000));
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < 6; i++) {
		unsigned int val;
		regmap_update_bits(priv->map, SPDIFIN_CTRL0,
				   SPDIFIN_CTRL0_STATUS_SEL_MASK,
				   SPDIFIN_CTRL0_STATUS_SEL(i));
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *c = snd_kcontrol_chip(kcontrol);
	struct axg_spdifin *priv = snd_soc_component_get_drvdata(c);
	unsigned int val, mode = 3;

	regmap_read(priv->map, SPDIFIN_STAT0, &val);
	mode = SPDIFIN_STAT0_MODE(val);

	if (mode < SPDIFIN_MODE_NUM)
		ucontrol->value.integer.value[0] =
			priv->conf->mode_rates[mode];
	else
